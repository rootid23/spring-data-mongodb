...
/**
 * {@link ReactiveExecutableRemoveOperation} allows creation and execution of reactive MongoDB remove / findAndRemove
 ...
 * @since 2.0
 */
public interface ReactiveExecutableRemoveOperation {

	/**
	 * Start creating a remove operation for the given {@literal domainType}.
	 *
	 * @param domainType must not be {@literal null}.
	 * @return new instance of {@link ReactiveRemoveOperation}.
	 * @throws IllegalArgumentException if domainType is {@literal null}.
	 */
	<T> ReactiveRemoveOperation<T> remove(Class<T> domainType);

	/**
	 * Compose remove execution by calling one of the terminating methods.
	 */
	interface TerminatingReactiveRemoveOperation<T> {
	 ...
	 */
	interface ReactiveRemoveOperationWithCollection<T> extends ReactiveRemoveOperationWithQuery<T> {
		 ...
		 * @return new instance of {@link ReactiveRemoveOperationWithCollection}.
		 * @throws IllegalArgumentException if collection is {@literal null}.
		 */
		ReactiveRemoveOperationWithQuery<T> inCollection(String collection);
	}

	/**
	 * Provide a {@link Query} override (optional).
	 */
	interface ReactiveRemoveOperationWithQuery<T> extends TerminatingReactiveRemoveOperation<T> {

		/**
		 * Define the query filtering elements.
		 *
		 * @param query must not be {@literal null}.
		 * @return new instance of {@link TerminatingReactiveRemoveOperation}.
		 * @throws IllegalArgumentException if query is {@literal null}.
		 */
		TerminatingReactiveRemoveOperation<T> matching(Query query);
	}

	interface ReactiveRemoveOperation<T> extends ReactiveRemoveOperationWithCollection<T> {}
}
