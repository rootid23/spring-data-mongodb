...
/**
 * Implementation of {@link ReactiveExecutableFindOperation}.
 *
 * @author Mark Paluch
 * @since 2.0
 */
@RequiredArgsConstructor
class ReactiveExecutableFindOperationSupport implements ReactiveExecutableFindOperation {
	...
	public <T> ReactiveFindOperation<T> query(Class<T> domainType) {
		...
		return new ReactiveFindOperationSupport<>(template, domainType, domainType, null, ALL_QUERY);
	}
	...
	static class ReactiveFindOperationSupport<T>
			implements ReactiveFindOperation<T>, ReactiveFindOperationWithCollection<T>,
			ReactiveFindOperationWithProjection<T>, ReactiveFindOperationWithQuery<T> {
		...
		@Override
		public ReactiveFindOperationWithProjection<T> inCollection(String collection) {

			Assert.hasText(collection, "Collection name must not be null nor empty!");

			return new ReactiveFindOperationSupport<>(template, domainType, returnType, collection, query);
		}

		@Override
		public <T1> ReactiveFindOperationWithQuery<T1> as(Class<T1> returnType) {
		...
		@Override
		public TerminatingReactiveFindOperation<T> matching(Query query) {
		...
		@Override
		public TerminatingReactiveFindNearOperation<T> near(NearQuery nearQuery) {
